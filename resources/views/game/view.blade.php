      ...
      fetchData() {
        fetch(`/api/game/{{ $game->id }}`)
            ...
            .then(data => {
                ...
                this.$nextTick(() => {
                    if (window.updateStatus) window.updateStatus(this.state);
                });
            });
        },
    },
...
// camera
const camera = new BABYLON.FreeCamera('camera', new BABYLON.Vector3(224, 50, 400), scene);
camera.setTarget(new BABYLON.Vector3(224, 0, 224));
...
// field (outfield)
const field = BABYLON.MeshBuilder.CreateGround('field', {width: 448, height: 448}, scene);
...
field.position = new BABYLON.Vector3(224, 0, 224);
...
const base1 = BABYLON.MeshBuilder.CreateBox('base1', {width: 3, height: 0.1, depth: 3}, scene);
base1.material = baseMaterial;
base1.position = new BABYLON.Vector3(224 + 63.63961030678928, 0.05, 280);
base1.addRotation(0, Math.PI / 4, 0);

const base2 = BABYLON.MeshBuilder.CreateBox('base2', {width: 3, height: 0.1, depth: 3}, scene);
...
const base3 = BABYLON.MeshBuilder.CreateBox('base3', {width: 3, height: 0.1, depth: 3}, scene);
base3.material = baseMaterial;
base3.position = new BABYLON.Vector3(224 - 63.63961030678928, 0.05, 280);
...
const fenceShape = [
    ...
    new BABYLON.Vector3(0.5, 10, 0),
    new BABYLON.Vector3(0.5, 0, 0)
];
const fencePath = [
    new BABYLON.Vector3(-72, 0, 50),
    new BABYLON.Vector3(50, 0, 35),
    new BABYLON.Vector3(224, 0, 25),
    new BABYLON.Vector3(398, 0, 35),
    new BABYLON.Vector3(520, 0, 50)
];
...
for (let pos = 1; pos <= 9; pos++) {
    // if (gameData.fielding && gameData.fielding[pos]) {
    //     const person = gameData.fielding[pos].person;
    ...
    //     texture.drawText(text, 0, 40, 'bold 24px Arial', 'black', 'transparent');
    //     const plane = BABYLON.MeshBuilder.CreatePlane('fielder' + pos, {width: 5, height: 2}, scene);
...
statusPlane.position = new BABYLON.Vector3(224, 85, 280);
...
window.updateStatus = function(state) {
    ...
    for (let i = 0; i < 4; i++) {
        ctx.beginPath();
        ctx.arc(lines[1].x + 5 + i * 13, 32, 6, 0, 2 * Math.PI);
    ...
    for (let i = 0; i < 3; i++) {
        ctx.beginPath();
        ctx.arc(lines[2].x + 20 + i * 13, 32, 6, 0, 2 * Math.PI);
    ...
    for (let i = 0; i < 3; i++) {
        ctx.beginPath();
        ctx.arc(lines[3].x + 6 + i * 13, 32, 6, 0, 2 * Math.PI);
